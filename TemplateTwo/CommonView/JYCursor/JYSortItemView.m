...
@interface JYSortItemView()

@property (nonatomic, weak) JYSortButton *selectButton;
@property (nonatomic, weak) JYSortButton *otherButton;
...
- (void)setupPositionViewsAndItemsWithKeys:(NSArray *)keys{
    ...
    for (NSString *title in keys) {
        JYSortButton *item = [[JYSortButton alloc]init];
        item.tag = num;
...
- (void)setSelectButtonTitle:(NSString *)selectButtonTitle{
    ...
    [self.itemsDic enumerateKeysAndObjectsUsingBlock:^(NSString *key, JYSortButton *button, BOOL *stop) {
...
- (JYSortButton *)getSortButtonWithTitle:(NSString *)title{
...
- (void)layoutItemsAfterDeletItem:(JYSortButton *)item{
    ...
    for (int i = index; i < self.tmpKeys.count-1; i++) {
        ...
        JYSortButton *nextButton = [self getSortButtonWithTitle:self.tmpKeys[i + 1]];
    ...
    [[JYItemManager shareitemManager] setItemTitles:self.tmpKeys];
...
- (void)longPress:(UILongPressGestureRecognizer *)longGesture{
    ...
    if (longGesture.state == UIGestureRecognizerStateBegan) {
        ...
        [[JYItemManager shareitemManager] setItemTitles:self.tmpKeys];
        [[JYItemManager shareitemManager] printTitles];
...
- (void)animationBetweenSelectItemIndex:(NSInteger)selectIndex AndOtherItemIndex:(NSInteger)otherIndex{
    ...
    if (selectIndex < otherIndex) {
        for (int i = (int)selectIndex + 1; i <= otherIndex; i++) {
            JYSortButton *item = [self.itemsDic objectForKey:self.tmpKeys[i]];
            [needMoveItem addObject:item];
            UIView *view = self.positionViews[i - 1];
            [positionView addObject:view];
        }
        int j = 0;
        for (JYSortButton *item in needMoveItem) {
        ...
        for (int i = j; i >= 0; i--) {
            ...
            JYSortButton *item = needMoveItem[i];
            [UIView animateWithDuration:0.3 animations:^{
                item.frame = view.frame;
            }completion:^(BOOL finished) {
                if (i == 0) {
...
- (void)itemsScare{
    self.isScareing = YES;
    [self.itemsDic enumerateKeysAndObjectsUsingBlock:^(NSString *key, JYSortButton *item, BOOL *stop) {
...
- (void)itemClick:(JYSortButton *)item{
    if (item.isScareing) {
...
- (JYSortButton *)getSortButtonWithKeyIndex:(NSInteger)index{
...
- (void)layoutSubviews{
    ...
    for (int i = 0; i < col ; i++) {
        ...
        for (int j = 0; j < RowNum ; j++) {
            ...
            JYSortButton *button = [self getSortButtonWithKeyIndex:count];
