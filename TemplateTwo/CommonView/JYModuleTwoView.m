...
- (JYCursor *)cursor {
    if (!_cursor) {
        ...
        //设置根滚动视图的高度
        _cursor.rootScrollViewHeight = self.frame.size.height - (45 + 64);
        //默认值是白色
        _cursor.titleNormalColor = JYColor_BackgroudColor_White;
        //默认值是白色
        _cursor.titleSelectedColor = JYColor_ThemeColor_LightGreen;
        _cursor.backgroudSelectedColor = JYColor_BackgroudColor_White;
        //是否显示排序按钮
        ...
        _cursor.backgroundColor = JYColor_ThemeColor_LightGreen;
    }
    return _cursor;
}
