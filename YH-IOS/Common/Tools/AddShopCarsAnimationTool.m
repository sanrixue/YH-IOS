...
- (void)startAnimationWithView:(UIView *)view EndRectPoint:(CGPoint)endRectPoint withfinishBlock:(finishAnimationBlock)finish{
    ...
    _layer.cornerRadius = view.frame.size.width*0.5;
    _layer.bounds = view.frame;
    
    //添加到窗口图层
    AppDelegate *appDelegate = (AppDelegate*)[UIApplication sharedApplication].delegate;
    [appDelegate.window.layer addSublayer:_layer];
    //定义起始位置 80是右边tableView的起始点
    _layer.position = [view convertPoint:view.center toView:appDelegate.window]; //CGPointMake(80+rect.origin.x+view.frame.size.width*0.5, CGRectGetMidY(rect));
...
+ (void)scaleAnimation:(UIView *)scaleView{
    ...
    CABasicAnimation *scaleAnimation = [CABasicAnimation animationWithKeyPath:@"transform.scale"];
    scaleAnimation.fromValue = [NSNumber numberWithFloat:1.2]; // 开始时的倍率
