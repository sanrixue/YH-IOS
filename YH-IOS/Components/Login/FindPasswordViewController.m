...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    else if(indexPath.row==2)
    {
        ...
        if (cell == nil) {//重新实例化对象的时候才添加button，队列中已有的cell上面是有button的
            ...
            NSString *str=@"如遇到手机号不匹配，请至OA办公系统-个人信息-我的信息中修改手机号码，隔天生效。";
            
            textLabel.numberOfLines=2;
            
//            [textLabel setLineSpacing:6];
...
//                make.top.mas_equalTo(cell.contentView.mas_topMargin).offset(16);
        ...
        if (cell == nil) {//重新实例化对象的时候才添加button，队列中已有的cell上面是有button的
            ...
            //添加按钮（或者其它控件）也可以写在继承自UITableViewCell的子类中,然后用子类来实例化这里的cell，将button作为子类的属性，这样添加button的触发事件的响应可以写在这里，target就是本类，方法也在本类中实现
                        ...
                        [upDataBtn addTarget:self action:@selector(upTodata) forControlEvents:UIControlEventTouchDown];
