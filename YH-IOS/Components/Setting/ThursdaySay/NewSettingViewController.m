...
- (void)viewDidLoad {
    [super viewDidLoad];
    userInfoArray = @[@"基本信息",@"选项配置",@"应用信息",@"消息推送",@"小四说"];
...
- (UIView *)tableFooterView {
    ...
    UIView *settingFooterView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 60)];
    UIButton *logoutBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 10, settingFooterView.frame.size.width, settingFooterView.frame.size.height - 20)];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    switch (indexPath.row) {
        case 0:{
            NSDictionary *infodict = @{@"邮箱":@"intfocus@gmail.com",@"电话":@"110110110"};
            SettingNormalViewController *settingNormalView = [[SettingNormalViewController alloc]init];
            settingNormalView.infodict  = infodict;
        ...
        case 2:{
            ...
            NSDictionary *infodict = @{@"名称":version.appName,@"型号":[NSString stringWithFormat:@"%@(%@)", version.current, version.build],@"设备型号":[NSString stringWithFormat: @"%@ (%@)",[[Version machineHuman]componentsSeparatedByString:@" ("][0], phoneVersion], @"数据接口":kBaseUrl,@"应用标识":version.bundleID,@"应用详情":@{@"检查新版本":@"升级到最新版",@"蒲公英下载":kPgyerUrl}};
            SettingNormalViewController *settingNormalView = [[SettingNormalViewController alloc]init];
            settingNormalView.infodict  = infodict;
            settingNormalView.title = userInfoArray[indexPath.row];
            [self.navigationController pushViewController:settingNormalView animated:YES];
